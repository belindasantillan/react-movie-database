            ...
            <div className="movies__background">
                <div className="movies__container">
                    <div className="movies__header">
                        ...
                        <div>
                            <input className="movies__search search" type="text" placeholder="Search by Keyword" onChange={(event) => setSearchKeyword(event.target.value)} onKeyUp={(event) => event.key === "Enter" && navigate(`/movies/${keyword}`)} />
                            <button className="movies__search--btn" disabled={!keyword} onClick={() => navigate(`/movies/${keyword}`)} ><FontAwesomeIcon icon="fa-solid fa-magnifying-glass" /></button>
                        </div>
                    </div>
                    <Link to={`/movies/${movieInfo.Title}`} className="movies__link">
